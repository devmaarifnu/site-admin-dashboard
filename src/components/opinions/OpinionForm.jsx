...
const opinionSchema = z.object({
  ...
  author_name: z.string().min(2, 'Nama penulis minimal 2 karakter').max(100, 'Nama penulis maksimal 100 karakter'),
  author_title: z.string().max(100, 'Jabatan penulis maksimal 100 karakter').optional(),
  author_bio: z.string().max(500, 'Bio penulis maksimal 500 karakter').optional(),
...
export default function OpinionForm({ initialData = null, mode = 'create' }) {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (mode === 'create') {
        await opinionsApi.create(formattedData);
        toast.success('Opini berhasil dibuat');
      } else {
        await opinionsApi.update(initialData.id, formattedData);
        toast.success('Opini berhasil diperbarui');
      }
      ...
      toast.error(error.response?.data?.message || 'Gagal menyimpan opini');
    } finally {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <Input
                  id="slug"
                  {...register('slug')}
                  placeholder="judul-opini-otomatis"
                  error={errors.slug?.message}
                  disabled={mode === 'edit'}
                />
                {mode === 'edit' && (
                  <p className="text-xs text-gray-500 mt-1">
                    Slug tidak dapat diubah setelah opini dibuat
                  </p>
                )}
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  {...register('excerpt')}
                  rows={3}
                  placeholder="Tulis ringkasan singkat opini..."
          ...
          <Card className="p-6">
            ...
            <div className="space-y-4">
              <div>
                <Label htmlFor="author_name" required>
                  Nama Penulis
                </Label>
                <Input
                  id="author_name"
              ...
              <div>
                ...
                <Input
                  ...
                  placeholder="Contoh: Ketua LP Ma'arif NU, Prof. Dr., dll"
                  error={errors.author_title?.message}
                />
              </div>

              <div>
                <Label htmlFor="author_bio">
                  Biografi Singkat
                </Label>
                <textarea
                  id="author_bio"
                ...
                />
                {errors.author_bio && (
                  <p className="text-sm text-red-600 mt-1">{errors.author_bio.message}</p>
                )}
              </div>
            </div>
          </Card>
